...
discharge$time <- time
vars <- c("Qtot.m3.s.","Vsup.Dt.m3.s.","Vsub.Dt.m3.s.","Vchannel.m3.") 
...
obs <- borden_obs_csv %>% read.table(sep=",",header=TRUE) %>% select(1:2)
names(obs) <- c("time","Qtot.m3.s.")
var_obs <- names(obs)[-1]
...
discharged <- discharge %>% filter(variable %in% var_obs)
obsc <- discharge %>% filter(sim=="obs") %>% transmute(time=time,variable=variable,obs=value)
pointd <- discharge %>% group_by(sim) %>% left_join(obsc) %>% as.data.frame()
